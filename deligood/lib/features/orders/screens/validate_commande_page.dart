...
class _ValidateCommandePageState extends State<ValidateCommandePage> {
  ...
  Future<void> submitCommande() async {
    ...
    try {
      await Api.post(
        '/api/orders/cart/',
        body: {'menu_item_id': widget.item.id, 'quantity': quantity},
      );

      await Api.post('/api/orders/client/create/');
      ...
      ScaffoldMessenger.of(
        context,
      ).showSnackBar(SnackBar(content: Text("Erreur lors de la commande\n$e")));
    }
  }
